...
class TinySpeck extends EventEmitter {
  ...
  send(/* ...args */) {
    ...
    // use defaults when available (THIS IS A LIE, don't use defaults -cc)
 ...
 /**
  ...
  listen(port, token) {
    ...
    return http.createServer((req, res) => {
      ...
      req.on('end', () => {
        let requestTime = req.headers["x-slack-request-timestamp"];
        let currentTime = new Date().getTime() / 1000;
        
        if (requestTime && currentTime - requestTime > 3) {
          console.log(`rejecting request because it's already timed out: requestTime=${requestTime}, currentTime=${currentTime}`);
        } else {
          ...
          res.end(JSON.stringify({ response_type: "in_channel" }));
        }
      });
